...
class ProtobufRPC:
    ...
    def __init__(self, transport: FlipperTransport, lock: Optional[asyncio.Lock] = None):
        ...
        Args:
            ...
            lock: Optional shared asyncio.Lock for coordinating with CLIBridge.
                  When provided, ``_send_rpc_message`` will acquire it to
                  prevent interleaved protobuf and CLI traffic.
        """
    ...
    async def _send_rpc_message(
        ...
        Flipper Zero RPC protocol uses nanopb-delimited framing:
        Send: [varint length][protobuf Main message]
        Receive: [varint length][protobuf Main response]

        When a shared lock is configured, this method acquires it to prevent
        interleaved traffic with CLIBridge.  The lock is skipped when already
        held (e.g. by CLIBridge during mode switching) because asyncio.Lock
        is not reentrant.
        ...
        """
        # Determine whether we need to acquire the lock.
        # When CLIBridge holds the lock it may call stop_rpc_session() or
        # ping() (for re-entry verification).  asyncio.Lock is NOT
        # reentrant, so we skip acquisition when the lock is already held.
        needs_lock = self._lock is not None and not self._lock.locked()
    ...
    async def stop_rpc_session(self) -> bool:
        """Send StopSession to cleanly exit RPC mode.

        The Flipper device will transition back to CLI mode after receiving
        this message.  On success, ``_rpc_session_started`` is reset so
        that the next RPC call will re-negotiate the session.

        Returns:
            True if the device acknowledged the stop with OK status.
        """
        try:
            main = flipper_pb2.Main()
            main.command_id = self._get_next_command_id()
            main.has_next = False
            main.stop_session.CopyFrom(flipper_pb2.StopSession())

            response = await self._send_rpc_message(main)
            if (
                response
                and response.command_status
                == flipper_pb2.CommandStatus.Value("OK")
            ):
                self._rpc_session_started = False
                return True
        except Exception:
            pass
        # Even if we didn't get a clean OK, mark session as stopped so
        # callers can attempt recovery.
        self._rpc_session_started = False
        return False
